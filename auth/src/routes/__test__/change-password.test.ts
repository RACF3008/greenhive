...
it("returns a 400 if there is no password given", async () => {
  ...
  await request(app)
    .post(`/api/users/change-password/${tokenId}`)
    .send({
      password: "",
      repeatPassword: "",
    })
...
it("returns a 400 if the passowrd doesn't meet the requirements", async () => {
  const tokenId = new mongoose.Types.ObjectId().toHexString();
...
it("returns a 400 if both passwords don't match", async () => {
  const tokenId = new mongoose.Types.ObjectId().toHexString();
...
it("returns a 200 if the password was changed succesfully", async () => {
  ...
  await request(app)
    .post(`/api/users/change-password/${resetToken.value}`)
    .send({
...
it("publishes a user:token-used event", async () => {
  const token = await global.signup();
